    ...
    fn read_rings<'a, F: WALStore + 'a>(
        ...
        futures::stream::unfold((), move |_| {
            ...
            async move {
                ...
                let header_raw =
                    match check!(v.file.read(v.off, msize as usize).await) {
                        Some(h) => h,
                        None => _yield!(),
                    };
                v.off += msize as u64;
    ...
    ) -> impl futures::Stream<Item = Result<(WALBytes, WALRingId, u32), bool>> + 'a
    {
        ...
        futures::stream::unfold((), move |_| {
            ...
            async move {
                ...
                loop {
                    let header_raw = match check!(
                        v.file.read(v.off, msize as usize).await
                    ) {
                        Some(h) => h,
                        None => _yield!(),
                    };
                    ...
                    match header.rtype.try_into() {
                        Ok(WALRingType::Full) => {
                            ...
                            let payload = check!(check!(
                                v.file.read(v.off, rsize as usize).await
                            )
                            .ok_or(()));
                        ...
                        Ok(WALRingType::Middle) => {
                            ...
                            if let Some((chunks, _)) = chunks {
                                let chunk = check!(check!(
                                    file.read(*off, rsize as usize).await
                                )
                                .ok_or(()));
                                if !check!(
                                    self.verify_checksum(&chunk, header.crc32)
